...
#define MQTT_ID "homeassistant/light-strip"
#define MQTT_CLIENT "192.168.1.145"
const char* MQTT_TOPIC_HUE = MQTT_ID "/hue";
const char* MQTT_TOPIC_SAT = MQTT_ID "/sat";
const char* MQTT_TOPIC_VAL = MQTT_ID "/val";
    ...
    void update()
    {
	    ...
	    if (mqtt
		    ...
		    // Connect
		    {
			    ...
			    options.clientID.cstring = (char*)"kids-bedroom-lights-2";
                options.username.cstring = (char*)"KidsBedroomLights2";
		    ...
		    }
		    {
			    ...
			    LOG_PRINTFLN("Subscribing to: %s", MQTT_TOPIC_HUE);
			    MqttClient::Error::type rc = mqtt->subscribe( MQTT_TOPIC_HUE, MqttClient::QOS0, m_cbMessageHandler );
			    if (rc != MqttClient::Error::SUCCESS) {
			    ...
			    LOG_PRINTFLN("Subscribing to: %s", MQTT_TOPIC_SAT);
			    rc = mqtt->subscribe( MQTT_TOPIC_SAT, MqttClient::QOS0, m_cbMessageHandler );
			    if (rc != MqttClient::Error::SUCCESS) {
				    LOG_PRINTFLN("Subscribe error: %i", rc);
				    LOG_PRINTFLN("Drop connection");
				    mqtt->disconnect();
         ...
         LOG_PRINTFLN("Subscribing to: %s", MQTT_TOPIC_VAL);
          rc = mqtt->subscribe( MQTT_TOPIC_VAL, MqttClient::QOS0, m_cbMessageHandler );
          if (rc != MqttClient::Error::SUCCESS) {
            LOG_PRINTFLN("Subscribe error: %i", rc);
            LOG_PRINTFLN("Drop connection");
            mqtt->disconnect();
